...
const Report = () => {
  const colors = ['red', 'orange', 'amber', 'yellow', 'lime', 'green', 'emerald', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose', 'slate', 'gray', 'zinc', 'neutral', 'stone', 'taupe', 'mauve', 'mist', 'olive'];
  const sizes = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950];
  
  const lin = (color, i) => {
    return (
      <div key={`color-${i}`} className="flex flex-col-1 items-center">
        <div className="grid min-w-0 flex-1 gap-x-1 sm:gap-x-4 grid-cols-12">
          <div className="flex items-center justify-end text-sm font-semibold capitalize text-gray-600 mb-4">
            {color}
          </div>
          { sizes.map((size, j) => 
            <div key={`size-${i}-${j}`}>
              <div className={`bg-${color}-${size} h-8 w-full rounded`} />
              <p className="text-xs text-center w-full">{size}</p>
            </div>
          ) }
        </div>
      </div>
