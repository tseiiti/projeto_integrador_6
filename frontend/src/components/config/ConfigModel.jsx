...
const ConfigModel = ({model, current, setCurrent}) => {
  ...
  const cur_aux = model.model == current ? 'blue' : 'gray';
  ...
  return (
    <div className={`relative flex flex-col bg-white border border-${cur_aux}-300 rounded-xl overflow-hidden transition-all hover:border-blue-600 hover:shadow-md group`}>
      <div className={`p-4 pb-2 border-b border-${cur_aux}-300 bg-${cur_aux}-100`}>
        ...
        <div className="flex items-center gap-2 mt-2">
          <span className={`text-[13px] font-bold text-${cur_aux}-500 group-hover:text-gray-500`}>{model.details.parameter_size}</span>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            { model.capabilities.map((c, i) => {
              return <span key={`span-capability-${i}`} className="material-symbols-outlined text-[16px] text-gray-500" title={c.charAt(0).toUpperCase() + c.slice(1)}>{capabilities[c]}</span>;
            }) }
          </div>
        </div>
